...
**Новые возможности:**
* Для всех команд управления сигнализацией реализован вывод в чат (опционально)
...
* В меню `[Q]->Утилиты->Metrostroi Advanced` добавлена панель "Клиент" с различными опциями
* В меню `[Q]->Утилиты->Metrostroi Advanced` добавлена панель "Админ" с возможностью управления почти всеми параметрами этого аддона, а также несколькоми серверными параметрами Метростроя: "Вкл. необходимость наличия КР", "Напряжение на КР", "Ограничение тока подстанции"
...
* При загрузке карты, точки телепорта на станциях автоматически центрируются для улучшения работы определения местоположения спавна
* Новый вид курсора в поезде по умолчанию, а также возможность сменить его, вернуть вид по умолчанию и даже отключить (в клиентской панели)
* Курсор в поезде автоматически исчезает при отсутствии движения мышью (можно отключить в клиентской панели)
...
* **metrostroi_advanced_signalsmessage 1** //вкл(1)/выкл(0) сообщение в чат об открытии/закрытии светофоров и маршрутов
...
* **metrostroi_advanced_fixpaths 0** //вкл(1)/выкл(0) автоматическое исправление номера пути в платформах
...
В комплект аддона входит пример прописанных кнопок пультов на карте **gm_metro_kalinin_v3**. Данные сохраняются в файл `metrostroi_advanced/map_buttons.txt`
...
* Объявления записаны 6-ю разными голосами.
